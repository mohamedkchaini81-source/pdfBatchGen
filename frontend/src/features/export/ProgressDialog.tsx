...
// Stop polling after this many consecutive errors or 404s
...
export function ProgressDialog({ onClose }: Props) {
      ...
      pollRef.current = setInterval(async () => {
        ...
        try {
          ...
          if (status.status === 'completed') {
            ...
            // Use API_BASE so the download goes to the correct backend in production
            setDownloadUrl(`${API_BASE}/exports/${jobIdRef.current}/download`)
            setPhase('done')
          ...
          // Stop polling on 404 (job gone — Render restarted) or repeated errors
          const is404 = e instanceof ApiError && e.status === 404
          if (is404 || errorCount.current >= MAX_POLL_ERRORS) {
            clearInterval(pollRef.current!)
            setPhase('failed')
            setMessage(
              is404
                ? 'Export job not found. The server may have restarted. Please try again.'
                : String(e)
            )
          }
        }
  ...
  async function handleCancel() {
    ...
    if (jobIdRef.current) {
      try { await cancelExport(jobIdRef.current) } catch (_) {}
    }
    setPhase('cancelled')
    setMessage(t('cancelled'))
  }
      ...
      <div className={styles.body}>
        ...
        >
          <div
            className={[
              styles.barFill,
              phase === 'failed' || phase === 'cancelled' ? styles.barError : '',
            ].join(' ')}
            style={{
